...
class StockPredictor:
    def __init__(self, db: Database):
        ...
        self._llm_accuracy_cache: Dict[str, float] = {}
    ...
    async def _enrich_with_web_search(self, ticker: str, db_news_context: str, research_callback=None) -> str:
        ...
        if cached and cached_age < self._web_search_cache_ttl:
            ...
            if raw_results:
                ...
                web_context = await summarize_search_results(ticker, raw_results)
                if web_context:
    ...
    async def build_feature_vector(self, ticker: str, as_of_date: str = None) -> Optional[dict]:
        ...
        # Add LLM Historical Accuracy (cached to avoid opening 1000+ DB connections during model training)
        if ticker in self._llm_accuracy_cache:
            llm_historical_accuracy = self._llm_accuracy_cache[ticker]
        else:
            llm_historical_accuracy = 0.5
            try:
                with self.db.connection() as conn:
                    cursor = conn.execute("SELECT is_correct FROM predictions WHERE ticker=? AND is_correct IS NOT NULL AND model_type='multi_agent' ORDER BY id DESC LIMIT 10", (ticker,))
                    results = cursor.fetchall()
                    if results:
                        correct = sum(1 for r in results if r[0] == 1)
                        llm_historical_accuracy = correct / len(results)
                self._llm_accuracy_cache[ticker] = llm_historical_accuracy
            except Exception:
                pass
            
        # Combine all features
        features = {**sentiment_features, **price_features, **market_features, "llm_historical_accuracy": llm_historical_accuracy}
        ...
        if etf:
            ...
            if etf_features and "return_1d" in etf_features:
                ...
                features["sector_etf_return_1d"] = 0.0
        else:
            features["sector_etf_return_1d"] = 0.0
    ...
    def _get_sentiment_features(self, ticker: str, as_of_date: str = None) -> dict:
        features = self.db.get_ticker_sentiment_features(ticker, lookback_days=7)
        if not features:
        ...
        def load_and_validate(path):
            ...
            try:
                ...
                if n_features is not None and n_features != expected_features:
                    log.warning(f"Model at {path} has {n_features} features, but expected {expected_features}. Discarding and retraining.")
                    try:
                        path.unlink()
                    except Exception:
                        pass
    ...
    def _get_sector(self, ticker: str) -> str:
        return self.db.get_ticker_sector(ticker)
    ...
    async def train_model(self, ticker: str, scope: str = "per_ticker", horizon_days: int = 1) -> Tuple[str, dict]:
        ...
        if len(np.unique(y)) >= 2:
            ...
            feature_names = sorted(list((await self.build_feature_vector(ticker)).keys()))
    ...
    async def _generate_narrative_with_confidence(self, ticker: str, features: dict, horizon_days: int, news_context: str = "") -> dict:
        ...
        try:
            ...
            prompt = (
                ...
                f"Respond with valid JSON (no markdown):\n"
                f'{{"direction": "UP" or "DOWN", "confidence": 0.0-1.0, '
                f'"narrative": "2-3 sentence explanation of the key factors and main risk"}}'
            )
            ...
            result["confidence"] = max(0.5, min(0.85, result["confidence"]))
            return result
        except Exception as e:
    ...
    def _get_model_path(self, name: str, horizon_days: int = 1) -> Path:
        return self.models_dir / f"{name}_model_{horizon_days}d.joblib"
