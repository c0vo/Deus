...
Two jobs live here, against that same endpoint:
  ...
  refresh_history()  daily OHLCV bars per ticker -> price_history, once a day.
...
class PriceFeed:
    ...
    async def refresh_history_for(
        ...
        """Fetch daily OHLCV for an explicit ticker list.

        refresh_history() is watchlist-only and pinned to a 3-month range.
        Thesis candidates are off-watchlist by construction, and the crowding
        score needs a year of bars to know where a name sits against its
        52-week high, so both of those assumptions have to be overridable.
        """
        ...
        stored = 0
        failed = 0
        for ticker, rows in zip(symbols, results):
            if not isinstance(rows, list) or not rows:
                failed += 1
                continue
            await asyncio.to_thread(self.db.upsert_price_history, ticker, rows)
    ...
    async def _fetch_history(
        ...
        """Fetch one ticker's daily OHLCV bars, oldest first."""
    ...
    async def _fetch_chart(
        ...
        """Fetch one ticker's chart payload as (meta, rows), oldest first.
        ...
        warm-up are one request rather than two.
